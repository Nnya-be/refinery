...
function Footer() {
  const footer_lists = {
    about: {
      title: "About",
      lists: ["Company", "Careers", "News"],
    },
    support: {
      title: "support",
      lists: ["Contact Support", "Help Resources", "Release Updates"],
    },
    platform: {
      title: "platform",
      lists: ["Terms & Privacy", "Pricing", "FAQ"],
    },
    contact: {
      title: "contact",
      lists: ["Send a Message", "Request a Quote", "+123-456-7890"],
    },
  };
  return (
    <div className="flex items-end w-full h-min bg-black">
      <footer className="w-full text-gray-400  body-font">
        <div className="container flex flex-col flex-wrap px-5 py-24 mx-auto md:items-center lg:items-start md:flex-row md:flex-no-wrap">
          <div className="flex-shrink-0 w-64 mx-auto text-center md:mx-0 md:text-left">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Clean Environment, Clean Reputation, Clean Sourcing
            </p>
            <div className="mt-4">
          ...
          <div className="flex flex-wrap flex-grow mt-10 -mb-10 text-center md:pl-20 md:mt-0 md:text-left">
            <div className="w-full px-4 lg:w-1/4 md:w-1/2">
              <FooterHeader heading={footer_lists.about.title}></FooterHeader>
              <nav className="mb-10 list-none">
                {footer_lists.about.lists.map((value) => {
                  return <FooterList list={value}></FooterList>;
                })}
              </nav>
            </div>
            <div className="w-full px-4 lg:w-1/4 md:w-1/2">
              <FooterHeader heading={footer_lists.support.title}></FooterHeader>
              <nav className="mb-10 list-none">
                {footer_lists.support.lists.map((value) => {
                  return <FooterList list={value}></FooterList>;
                })}
              </nav>
            </div>
            <div className="w-full px-4 lg:w-1/4 md:w-1/2">
              <FooterHeader
                heading={footer_lists.platform.title}
              ></FooterHeader>
              <nav className="mb-10 list-none">
                {footer_lists.platform.lists.map((value) => {
                  return <FooterList list={value}></FooterList>;
                })}
              </nav>
            </div>
            <div className="w-full px-4 lg:w-1/4 md:w-1/2">
              <FooterHeader heading={footer_lists.contact.title}></FooterHeader>
              <nav className="mb-10 list-none">
                {footer_lists.contact.lists.map((value) => {
                  return <FooterList list={value}></FooterList>;
                })}
              </nav>
            </div>
        ...
        <div className="bg-gray-400">
          <div className="container px-5 py-4 mx-auto">
            <p className="text-sm text-gray-700 capitalize xl:text-center">
              GG REFINERY LIMITED. © {new Date().getFullYear()} All rights
              reserved{" "}
            </p>
          </div>
        </div>
...
// function Footer() {
//   return (
//     <footer className="min-h-screen bg-gray-800 text-white p-4">
//       <div className="container mx-auto text-center">
//         <p>Company contacts and details...</p>
//       </div>
//     </footer>
//   );
// }
